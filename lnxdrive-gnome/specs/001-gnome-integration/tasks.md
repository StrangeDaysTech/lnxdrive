...
- [x] T039 [US2] Implement D-Bus error handling in menu action callbacks in `lnxdrive-menu-provider.c`: in `_pin_file` async callback: catch `com.strangedaystech.LNXDrive.Error.InsufficientDiskSpace` → send `GNotification` via `g_application_send_notification()` with title "Not Enough Disk Space" and body "Cannot download file — insufficient disk space available" (FR-036). In `_unpin_file` async callback: catch `com.strangedaystech.LNXDrive.Error.FileInUse` → send `GNotification` "File In Use" / "Cannot free space — file is being used by another application" (FR-037). Catch `InvalidPath` → "File not in sync folder". Catch generic `GError` → "LNXDrive: Operation failed" with error message. Ref pending issue FR-008: GNotification provides explicit visual feedback beyond overlay icon change.
